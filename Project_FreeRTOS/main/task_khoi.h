...
void task_KHOI(void *pvParameters)
{
  ...
  {   
    ...
    if(khoi_currentvalue<khoi_alarm1)
    {
        sendMessage_2(IN_MQ_135, OUT_FAN, FAN_MQ_DISABLE, Khoi_Queue);
    ...
    {
        sendMessage_2(IN_MQ_135, OUT_FAN, FAN_MQ_ENABLE_MEDIUM, Khoi_Queue);
    ...
    {
        sendMessage_2(IN_MQ_135, OUT_FAN, FAN_MQ_ENABLE_HIGH, Khoi_Queue);
...
void sendMessage_2(ModuleID id_Tx, ModuleID id_Rx, ActionPayload payload, QueueHandle_t queue)
{
    Message mq_send {INVALID_MODULEID, INVALID_MODULEID, INVALID_ACTIONPAYLOAD};
    mq_send.id_Tx = id_Tx;
    mq_send.id_Rx = id_Rx;
    mq_send.payload = payload;
    if (xQueueOverwrite(queue, &mq_send) == pdPASS)
    {
